package mip.sys.deserted

import chisel3._
import mip.MipConfigs.N_MIP_CHANNELS
import mip.sys.CtrlReg
import mip.units._
import mip.xilinx._
...
class MipSystem extends Module {
    ...
    object SysStates extends ChiselEnum {
        val IDLE, CALC, UPDATE, DELAY = Value
    }
    val delayCnt = RegInit(0.U(64.W))
    ...
    // vram.io.calcResult <> calcUnit.map(_.out) // FIXME: decomment this line
    vram.io.VramRead <> io.vram_port
