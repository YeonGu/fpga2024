...
class MipSys extends Module {
    ...
    object SysState extends ChiselEnum {
        val IDLE, CALC, UPDATE, WAIT = Value
    }
    val state = RegInit(SysState.IDLE)
    val allFinished = dataFetcher.io.ctrl.dispatchFifoRdens === 4194304.U && calcUnit.map(
        _.mipCtrl.queueEmpty
    ).reduce(_ && _)

    val waitCnt   = RegInit(0.U(32.W))
    val waitThres = 1024.U
    ...
    switch(state) {
        ...
        is(SysState.CALC) {
            when(allFinished) { state := SysState.WAIT }
            waitCnt := 0.U
        }
        is(SysState.UPDATE) {
            state := SysState.WAIT
        ...
        is(SysState.WAIT) {
            when(!allFinished) { state := SysState.CALC }
                .elsewhen(waitCnt === waitThres - 1.U) { state := SysState.UPDATE }
            waitCnt := waitCnt + 1.U
        }
    }
